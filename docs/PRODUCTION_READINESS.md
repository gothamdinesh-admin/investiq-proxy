...
3. **Resend delivery — CONFIRMED in test mode (2026-07-04).** The Harbour Test Signup Email returned `Resend 403: You can only send testing emails to your own email address (gothamdinesh@gmail.com)`. Resend has no verified domain, so it will send ONLY to gothamdinesh@gmail.com and ONLY from `onboarding@resend.dev`. Two-step resolution:
   - **Now (unblocks admin signup notifications):** in the Harbour project's Edge Function secrets set `ADMIN_NOTIFY_EMAILS=gothamdinesh@gmail.com` and `RESEND_FROM_EMAIL=InvestIQ <onboarding@resend.dev>`, then re-run the test. The "someone signed up" email to you works; approval-to-user emails stay dark (users just see the pending→approved gate flip when they retry). Approving a user already works without the email (it's best-effort, non-blocking).
   - **Production (approval emails reach Harbour staff):** verify a Resend sending domain — ideally one you personally control (fast, no IT dependency) or `iq.harbourasset.co.nz` via Harbour IT (SPF/DKIM/DMARC). Then set `RESEND_FROM_EMAIL` to that domain. Until then, approval emails to non-gmail addresses will 403.
