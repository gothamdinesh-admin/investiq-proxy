...
- [~] **Multi-portfolio support** — _v0.21a + v0.21b SHIPPED._
  ...
  - **v0.21.1 (HOTFIX)** — fixed multi-portfolio data loss: a stale PostgREST schema cache rejected writes to the new `portfolios` column, and the fallback silently saved only the active portfolio → renames/extra portfolios vanished on reload. Three fixes: (1) **settings stash** — the envelope is also written to `settings._portfolios`, an always-writable transport that doesn't depend on the migration columns or schema cache; (2) **structure guard** in `loadFromSupabase` — never collapse a multi-portfolio local into a single-portfolio cloud; (3) migration 016 now runs `NOTIFY pgrst, 'reload schema'`. _Re-run migration 016 (or run `NOTIFY pgrst, 'reload schema';` once) so the dedicated column writes directly too._
  - **v0.21c** — "All portfolios (combined)" view + CSV import target picker. The switcher gets an "All portfolios" option that shows a merged net-worth view across every portfolio. It is **read-only at the data layer**: the `state.portfolio` accessor returns merged (tagged) holdings and the setter is a no-op while combined, so it physically cannot mutate the underlying portfolios. The flag is transient (never persisted) and a banner + edit-guards (saveHolding / deleteHolding / editHolding / populateAddForm / importCSV) make the read-only state clear. CSV import now asks which portfolio to import into when more than one exists.
  - **v0.21d** — per-portfolio snapshots, **no schema change**. Snapshots now store the account-wide total at the top level (so the dropdown, weekly digest, and DR tools see true net worth) plus a per-portfolio breakdown + portfolio-tagged holdings inside the existing `breakdown` JSONB. Performance chart + KPI strip + Time Travel (table, header, dropdown) all scope to the active portfolio via `loadSnapshotHistory`/`_scopeSnapshotRows`; combined view shows the account total. Legacy snapshots attribute to the original 'default' portfolio only. `getMetrics(holdingsOverride)` added so each portfolio's totals compute in isolation. Snapshot-restore now rebuilds the full multi-portfolio structure from tagged holdings. **No migration 017 needed; Edge Functions untouched.**
  - **v0.21e** — family aggregate. The household total already sums each member's full net worth across portfolios (v0.21d made member snapshot totals account-wide). v0.21e surfaces each member's **per-portfolio split** as chips (from `breakdown.portfolios`) in their card, and labels the household value as spanning all portfolios. Read-only render change, no schema/query change.
  - ✅ **Multi-portfolio epic COMPLETE** (v0.21a → v0.21e). Switcher, cloud-resilient envelope, per-portfolio AI/tax (FIF per-person), combined view, per-portfolio snapshots, family per-portfolio split.
