...
- **Phase 2.5 — DONE (v0.41): brand name/tagline + chart titles editable.** `brand.name` / `brand.tagline` overrides (via `cmsName()`/`cmsTagline()`) drive the document title, meta/PWA tags, the `.edition-name` slots, the header **wordmark**, and `{name}`/`{tagline}` interpolation everywhere — a true rebrand from the Studio. `applyEdition()` re-runs on `brand.*` save/reset. Allocation/Performance chart titles editable (`chart.allocTitle`/`chart.perfTitle`). Verified: setting `brand.name` → title + tab + wordmark all update; personal defaults unchanged.
- **Phase 2.6 — DONE (v0.43): navigation & section visibility.** New "Navigation & sections" Studio group (`nav.group.*` + `nav.section.*` toggles). `applyCmsNav()` hides/shows sidebar groups, mobile drawer items and bottom tabs per override; `isCmsSectionHidden()`/`isCmsGroupHidden()` are the resolvers (a section is hidden if its own toggle OR its parent group is off; a group is hidden if its toggle is off OR all its children are). `renderGroupSubNav()` drops hidden children from the pill bar, `showGroup()` lands on the first *visible* child, and `_showSectionCore()` bounces a hidden section to Overview so it's truly unreachable. overview/admin/family are not CMS-controlled (always-on / role-gated). Runs at startup (in `loadCmsOverrides`) and live on each toggle. **Verified** against the real DOM: hiding the Watchlist group removes its sidebar link; hiding Market drops it from the Intelligence pills and `showGroup('intelligence')` lands on Insights.
- **Phase 2+ — TODO:** keep widening (per-chart colours, more section subtitles/buttons/empty-states).
  ...
  - **Feature flags & nav** — ~~`NAV_GROUPS` / section visibility~~ DONE (2.6). Remaining: `EDITION.features` surfaced as toggles, editable `SECTION_LABELS/ICONS`.
